...
public class TreeImpl<E extends Comparable<E>> implements Tree<E> {
    ...
    public boolean add(E value) {
        ...
        if (nodeAndParent.level >= this.maxLevel) {
            return false;
        }
    ...
    @Override
    public boolean traverse(TraversMode mode) {
        switch (mode) {
            ...
            case PRE_ORDER_BALANCE -> {
                return checkBalance();
            } //прямой обход с проверкой балансировки
        }
        System.out.println();
        return true;
    }
    ...
    private boolean checkBalance() {
        preOrderCheckBalance(root);
        return maxLevel - minLevel <= 1;
    }

    private void preOrderCheckBalance(Node<E> current) {
        if (current == null) {
            return;
        }

        if (current.getLeftChild() == null || current.getRightChild() == null) {
            minLevel = Math.min(minLevel,doFind(current.getValue()).level);
            //System.out.print(current.getValue() + " (" + doFind(current.getValue()).level + ") ");
        }

        preOrderCheckBalance(current.getLeftChild());
        preOrderCheckBalance(current.getRightChild());
